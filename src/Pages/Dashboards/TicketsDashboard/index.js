...
const TicketDashboard = () => {
  ...
  const { data: tickets } = useQuery(GET_ALL_TICKETS_QUERY, {
    variables: { input: { isClosed: false }, page: 0 },
  });
  ...
  const getTasksId = (status, data, searchText) => {
    const tasks = [];
    let filteredByStatus = data?.filter(
      (item) => item?.status?.id === status.id
    );
    if (searchText) {
      const search = (ticket) => {
        const username =
          ticket?.raisedBy?.firstName?.toLowerCase() +
          ticket?.raisedBy?.lastName?.toLowerCase();
  ...
  const onDragEnd = (result) => {
    ...
    if (
      data?.columns[source.droppableId].name == "New" &&
      data?.columns[destination.droppableId].name == "In-progress"
    ) {
      setIsModalVisible(true);
      return;
    }
    if (
      data?.columns[source.droppableId].name == "In-progress" &&
      data?.columns[destination.droppableId].name == "Resolved"
    ) {
      setIsModalVisible(true);
      return;
    }
    ...
    updateTicket({
      variables: {
        updateTicketId: draggableId,
        input: { status: destination.droppableId },
      },
    });
  };
    ...
    <>
      ...
      {showTicketDeatils && (
        <Modal
          ...
          visible={showTicketDeatils}
          onOk={handleCloseTicketDetails}
          onCancel={handleCloseTicketDetails}
        >
